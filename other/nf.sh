...
function MediaUnlockTest_Sooka() {
    echo -n -e " Sooka\t\t\t\t           ->\c"
    local tmpresult=$(curl -${1} -s "https://www.sooka.com/")
    
    local region="Unknown"
    if echo "$tmpresult" | grep -qi "Malaysia"; then
        region="MY"
    fi
    ...
    if echo "$tmpresult" | grep -qi "sorry\|not available\|restricted\|unavailable"; then
        echo -n -e "\r Sooka\t\t\t\t: ${Font_Red}No Access${Font_Suffix} (Region: ${region})\n"
        return
    fi

    if echo "$tmpresult" | grep -qi "watch now\|login\|browse\|premium"; then
        echo -n -e "\r Sooka        \t\t\t\t: ${Font_Green}Yes (Region: ${region})${Font_Suffix}\n"
        return
    fi

    echo -n -e "\r Sooka\t\t\t\t: ${Font_Red}Failed${Font_Suffix}\n"
}
...
function MediaUnlockTest_TikTok() {
    ...
    local http_status
    http_status=$(curl -4 -s -o /dev/null -w "%{http_code}" -L https://www.tiktok.com/)

    local region
    region=$(curl -4 -sI https://www.tiktok.com/ | grep -i 'x-tt-geo-region:' | awk '{print $2}' | tr -d '\r')

    if [ -z "$region" ]; then
        region=$(curl -4 -s ipinfo.io/country)
    fi

    if [[ "$http_status" =~ ^2 ]]; then
        echo -e "\r TikTok       \t\t\t\t: ${Font_Green}Yes (Region: ${region})${Font_Suffix}"  # Removed \n
    else
        echo -e "\r TikTok\t\t\t\t: ${Font_Red}No${Font_Suffix}"  # Removed \n
    fi
}

function MediaUnlockTest_Shopee() {
    local ipver=${1:-4}
    echo -ne " Shopee\t\t\t\t   \c"

    local headers
    headers=$(curl -$ipver -s -I -L --max-redirs 3 -m 8 https://shopee.com/ 2>/dev/null)

    if [[ $? -ne 0 || -z $headers ]]; then
        echo -ne "\r Shopee\t\t\t\t: \e[31mFailed (Network Connection)\e[0m\n"
        return
    fi

    local final_url
    final_url=$(echo "$headers" | awk 'tolower($1)=="location:" {print $2}' | tail -n 1 | tr -d '\r')

    # Improved region extraction logic
    local region
    region=$(echo "$final_url" | awk -F'.' '{if ($NF == "com") print $(NF-1); else print $NF}' | awk -F'/' '{print $1}' | tr -d '\r')
    region=${region^^}  # Convert to uppercase

    local status
    status=$(curl -$ipver -s -o /dev/null -w "%{http_code}" -L --max-redirs 3 -m 8 https://shopee.com/)

    if [[ $status != 2* && $status != 3* ]]; then
        echo -ne "\r Shopee\t\t\t\t: \e[31mNo Access\e[0m (Region: $region)\n"
        return
    fi

    echo -ne "\r Shopee       \t\t\t\t: \e[32mYes (Region: $region)\e[0m\n"
}

function MediaUnlockTest_Facebook() {
    echo -n -e " Facebook\t\t\t\t->\c"

    # Check HTTP status
    local http_status
    http_status=$(curl -${1} -s -o /dev/null -w "%{http_code}" -L https://www.facebook.com/)

    # Extract region properly
    local region
    region=$(curl -${1} -sI https://www.facebook.com/ | grep -i 'x-fb-geo' | awk '{print $2}' | tr -d '\r')

    # Fallback to IP-based country detection if headers don't return a valid region
    if [ -z "$region" ] || [[ "$region" =~ [^A-Za-z] ]]; then
        region=$(curl -${1} -s ipinfo.io/country)
    fi

    # Determine access status
    if [[ "$http_status" =~ ^2 ]]; then
        echo -ne "\r Facebook\t\t\t\t: ${Font_Green}Yes (Region: ${region})${Font_Suffix}\n"
    else
        echo -ne "\r Facebook\t\t\t\t: ${Font_Red}No${Font_Suffix}\n"
    fi
}
...
function global() {
    ...
    MediaUnlockTest_Shopee ${1}
    MediaUnlockTest_Facebook ${1}
}
